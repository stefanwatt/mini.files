...
local workspace = require("mini.lsp.workspace")

local M = {}

---@param actions mini_files.FS_Action[]
---@return fun() did_perform Call this function when the file operations have been completed
function M.will_perform_file_operations(actions)
  ...
  for _, action in ipairs(actions) do
    if action.type == "move" then
      local src_scheme, src_path = utils.parse_url(action.src_url)
      assert(src_path)
      local src_adapter = assert(utils.get_adapter_by_scheme(src_scheme))
      local dest_scheme, dest_path = utils.parse_url(action.dest_url)
      local dest_adapter = assert(utils.get_adapter_by_scheme(dest_scheme))
      src_path = utils.posix_to_os_path(src_path)
      dest_path = utils.posix_to_os_path(assert(dest_path))
    ...
    elseif action.type == "create" then
      local scheme, path = utils.parse_url(action.url)
      path = utils.posix_to_os_path(assert(path))
      local adapter = assert(utils.get_adapter_by_scheme(scheme))
    ...
    elseif action.type == "copy" then
      local scheme, path = utils.parse_url(action.dest_url)
      path = utils.posix_to_os_path(assert(path))
      local adapter = assert(utils.get_adapter_by_scheme(scheme))
      if adapter.name == "files" then
        table.insert(creates, path)
  ...
  -- local timeout_ms = config.get_config().lsp_file_methods.timeout_ms
